...
begin
  ...
  c_par:=length(ar_w);
  arr:=nil;
  // Новая версия определение номера команды
  j:=length(help);
  n_com:=-1;
  for i:=0 to j-1 do
  ...
  if (c_par>1) and (n_com in [0..3,5,9,10,13]) and
    ...
    raise Einouterror.Create('Ошибка в имени матрицы.');
//
  case n_com of
  ...
  0: begin
      if c_par>=4 then begin
                      ...
                      val(ar_w[2], n, code);
                      if code <> 0 then
                       raise EinoutError.Create('вы ввели не число');
                      val(ar_w[3], m, code);
                      if code <> 0 then
                       raise EinoutError.Create('вы ввели не число');
  ...
  2: begin
      if c_par>=2 then begin
                         ...
                         if (ar_dat[id_m].ar=nil) then
                          raise EinoutError.Create('Матрица отсутствует');
                         fname:='';
                         if c_par=3 then fname:=ar_w[2];
                       end else
      begin
      write('Введите Id_m :');
      readln(id_m);
      if ar_dat[id_m].ar=nil then  raise EinoutError.Create('Матрица отсутствует');
 ...
 5: begin
      if (c_par <> 3 ) then raise Einouterror.Create('ошибка в параметрах ');
      c1 := ar_w[1][1];
      c:= ar_w[2][1];
      if not((c in ['A'..'Z']) and (c1 in ['A'..'Z'])) then raise Einouterror.Create('ошибка в параметрах ');
      if ar_dat[c].ar = nil then  raise Einouterror.Create('матрица пуста ');
      id_m := c1;
  ...
  7: begin
        s0:='test_in';//const
        s1:='Проверка механизма ';//copy
        s2:='передачи параметров';//var
        s3:=s1+s2; //out
        writeln(s3,' Adr out s3[1] в прг до = ',uint32(@s3[1]));
        ...
        test(s0,s1,s2,s3,i);
        writeln(s3,' Adr out s3[1] в прг после= ',uint32(@s3[1]));
        writeln('i= после=',i,' adr i=',int32(@i));
     end;
  //Задание параметров для генератора diap*random+k
  8: begin
     if c_par=3 then begin
      ...
      readln(diap);
      write('Введите значение "подставки" k_r : ');
      readln(k_r);
     end;
     // Генерация целочисленной матрицы
  9: begin
      if c_par>=4 then begin
                      id_m:=ar_w[1][1];
                      n:=strtoint(ar_w[2]);
 ...
 10: begin
     if (c_par=6) and (ar_w[2][1]= '=') then begin //OK
         ...
         if not (ar_w[4][1] in ['+','-','*','/']) then
           raise Einouterror.Create('Ошибка в операции');
         c:=ar_w[4][1]; // operation
         end
     else raise Einouterror.Create('Ошибка в количестве параметров или нет =');
     //
     case c of
    ...
    '*','/':begin
        ...
        // матриц * матрица
        'A'..'Z': begin
          if c = '/'  then raise Einouterror.Create('ошибка в операции');
        ...
        'a'..'z':if ar_dat[ansiuppercase(ar_w[5])[1]].kind>=2 then
                        ...
                        if (c = '/') then if (abs(el) > 1e-8) then el := 1 / el
                                else raise Einouterror.Create('нельзя делить на ноль');
                        arr:= sc_mult_m(ar_dat[ar_w[3][1]].ar,el,count_op);
                        end
                  else raise Einouterror.Create('В этой матрице нет скаляра')
        else begin
             val(ar_w[5],el,code);
             if code<>0 then
                raise Einouterror.Create('Ошибка в скалярном множителе');
             if (c = '/') then if (abs(el) > 1e-8) then el := 1 / el
                                else raise Einouterror.Create('нельзя делить на ноль');
             arr:= sc_mult_m(ar_dat[ar_w[3][1]].ar,el,count_op);
             end;
 ...
 11: begin
     writeln('Список матриц',#10,'Имя',#9,'n',#9,'m');
     for c := 'A' to 'Z' do
       begin
       rec:=ar_dat[c];
 ...
 12: begin
     writeln('Список скалярных значений',#10,'Имя',#9,'значение');
     for c := 'A' to 'Z' do
       begin
       rec:=ar_dat[c];
       if rec.kind>=2 then
 ...
 13: begin
      if (c_par <> 4) then raise Einouterror.Create('ошибка в параметрах!!');
 ...
 14: begin
  ...
  //Генерация единичной матрицы размера n, n - целое число
 15: begin
      if c_par <> 3 then  raise Einouterror.Create('ошибка в параметрах!!');
 ...
 // Метод обратной подстановки (решение) для выходной матрицы
 17: begin
       if  c_par <> 3 then  raise Einouterror.Create('ошибка в параметрах!!');
       id_m := ar_w[1,1];
 ...
 18: begin
      if c_par <2 then   raise Einouterror.Create('ошибка в параметрах!!');
      k :=  strtoint(ar_w[1]);
      d :=  strtoint(ar_w[2]);
      if c_par = 4 then
        cfile :=  ar_w[3,1];
 ...
 // LU
 20: begin
       if c_par<>4 then raise Einouterror.Create('Ошибка в команде');
       id_m:=ar_w[1][1];
       c:= ar_w[2][1];
       c1:=ar_w[3][1];
       ar1:=nil;
       ...
       i:=n*(n*n-1) div 3;
       writeln('Теоретическое count_op=',i);
     end;
 21: begin
       if c_par<>4 then raise Einouterror.Create('Ошибка в команде');
       id_m:=ar_w[1][1];
 ...
 // LUP
 24: begin
       if c_par<>5 then raise Einouterror.Create('Ошибка в команде');
       id_m := ar_w[1][1];
       c:= ar_w[2][1];
       c1:=ar_w[3][1];
       ar1:=nil;
 ...
 25: begin
  ...
  //
  { TODO : добавить  редуцирование матрицы в скаляр }
  except
  on E:EinoutError do
    if E.ErrorCode=103 then writeln('Файл ',fname,' не найден')
    ...
    if length(arr)*length(arr[0])=1 then
       begin
         ...
         ar_dat[id_m].f:=arr[0,0];
          ...
          ar_dat[id_m].ar:=nil;//иначе утечка памяти !
